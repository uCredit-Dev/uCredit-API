...
router.get("/api/search/all", async (req, res) => {
  try {
    const courses = await SISCourseV.find({}).exec();
...
router.get("/api/search/skip/:num", async (req: any, res) => { // todo: change req type
  const toSkip: number = +req.params.num;
...
router.get("/api/searchNumber/:number", async (req, res) => {
  ...
  try {
    let courses = await SISCourseV.find({ number }).exec();
    if (courses.length == 0) {
      ...
      courses = await SISCourseV.find({ number }).exec();
      if (courses.length == 0) {
...
router.get("/api/search", async (req: any, res) => { // todo: change req type
...
//return all versions of the course based on the filters
router.get("/api/cartSearch", async (req: any, res) => { // todo: change req type
    // page is defined or 0
    const PERPAGE = 10; 
    const page = parseInt(req.query.page) || 0;
    const expr = req.query.expr; 
    let result: any = {}; // todo: check the type of this
    try {
      if (expr.length === 0) {
        result.courses = await SISCourseV.find({}).skip(page * PERPAGE).limit(PERPAGE).exec(); 
        result.pagination = {
          page: page, 
          limit: PERPAGE, 
          last: 10,
...
router.get("/api/getYearRange", async (req, res) => {
  ...
  try {
    const resp = await SISCourseV.distinct("terms");
